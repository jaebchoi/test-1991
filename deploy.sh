...
startup() {
  ...
  helm upgrade --install $INFRA_NAME test-1991-infrastructure \
     --values test-1991-infrastructure/values.yaml \
     --values test-1991-infrastructure/values-dev.yaml
  if ! kubectl rollout status --namespace argocd deployment/argocd-server --timeout=30s; then
    exit $?
  fi
  argocd repo add https://github.com/jaebchoi/test-1991 --server localhost:30080 --plaintext --insecure-skip-server-verification
}
...
deploy() {
  ...
  if is_app_running; then
    echo "test-1991 is deployed"
  else
    branch=$(git rev-parse --abbrev-ref HEAD)
    echo "Deploying test-1991 from branch '$branch'..."
    argocd app create $APP_NAME \
      --server localhost:30080 --plaintext \
      --dest-namespace test-1991 \
      --dest-server https://kubernetes.default.svc \
      --repo https://github.com/jaebchoi/test-1991 \
      --path test-1991-deploy/src/main/resources \
      --revision $branch \
...
down() {
  if is_app_running; then
    ...
    echo "test-1991 is not deployed"
  fi
}
